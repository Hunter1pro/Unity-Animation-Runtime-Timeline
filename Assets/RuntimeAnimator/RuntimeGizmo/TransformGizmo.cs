...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		void OnEnable()
		{
            ...
            {
                UnityEngine.Rendering.RenderPipelineManager.endFrameRendering += RenderPipelineManager_endFrameRendering;
            }
			else
            {
				Camera.onPostRender += BasicOnPostRender_Call;
			}
		}
		...
		private void BasicOnPostRender_Call(Camera camera)
        {
			//if (this.CheckFilter(camera))
			//{
				this.OnPostRender_Call();
            //}				
        }

		private void RenderPipelineManager_endFrameRendering(UnityEngine.Rendering.ScriptableRenderContext arg1, Camera[] arg2)
		{
			this.OnPostRender_Call();
		}
		...
		void Update()
		{
            ...
            {
				this.copyPoss = this.mainTargetRoot.localPosition;
				this.copyRotation = this.mainTargetRoot.localRotation;
            }

			if (Input.GetKey(KeyCode.LeftControl) && Input.GetKeyDown(KeyCode.V))
            {
				if (this.copyPoss != Vector3.zero)
                {
					this.mainTargetRoot.localPosition = this.copyPoss;
					this.mainTargetRoot.localRotation = this.copyRotation;
                }
            }
		}
